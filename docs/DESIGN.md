...
a hazard is an immovable obstacle, so it can only ever lose a tie, and it never
raised the longest par the search could reach. The rule is still in the engine,
still documented, and still exercised — `tools/qa.js` builds a board for it.
...
Rounding to whole moves is the only thing allowed to bend that line, so `npm
test` requires every stage to sit within half a move of it, and requires par to
never decrease from one stage to the next. Roughly three stages share each par;
each takes the next-best board of that length, so no puzzle ships twice.

## The obstacle budget

Longest par found, by how much was added to an empty tray:
...
Two things this table settles. Obstacles do not simply make boards longer — a
fourth wall makes them *shorter*, because it starts removing the room a long
solution needs. And a single drifter buys more length than a third wall, which
is why the long end of the campaign is one wall or two plus one drifter rather
than a tray full of walls.
