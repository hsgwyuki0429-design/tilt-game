...
(function () {
  ...
  var renderer = null, reactions = null, autoplay = null;
  ...
  function show() {
    ...
    history = [];
    busy = false;
    reactions.setStage(stage, function () { return state; });
    reactions.reset();
  ...
  function move(dir) {
    ...
    if (!res.moved) {
      renderer.rebuff(dir);
      reactions.begin().proposeLive('miss', { dir: dir }).commit();
      return;
    }
    var before = state;
  ...
  /**
   * Let the penguins pull the face the game would pull.
   *
   * The full reaction table lives in Game — chain reactions, near misses,
   * standing next to cracked ice — and reimplementing it here would be a second
   * copy of it to keep in step. What this needs is the part that changes how a
   * BOARD reads while you are judging it: the solver's verdict on the move you
   * just made, and whether the board is now clear or lost. Everything else the
   * penguins do is the game's business, and src/expression.js is where it is
   * tested.
   */
  function react(before, res) {
    var verdict = X.evaluateMove(before, state, {
      stage: stage,
      beforeDist: distanceOf(before),
      afterDist: distanceOf(state)
    });
    reactions.begin();
    if (verdict.confidence > 0 && verdict.type !== 'normal') reactions.proposeLive(verdict.type);
    if (res.clear) reactions.proposeAll('clear');
    else if (res.broken) reactions.proposeAll('fail');
    reactions.commit();
  }

  /** What the solver says this position is still worth, in the shape evaluateMove wants. */
  function distanceOf(st) {
    var r = E.solve(stage, st, 200000);
    return { solvable: !!r.solvable, exact: !r.truncated, moves: r.moves };
  }
  ...
  reactions = new X.PenguinReactions();
  renderer.reactions = reactions;
  bind();
  requestAnimationFrame(loop);
